...
class DeviceSpec(_system: ActorSystem)
  ...
  "" should "return TemperatureNotAvailable for devices with no readings" in {
  ...
  "" should "return DeviceNotAvailable if device stops before answering" in {
    val requester = TestProbe()
  ...
  "" should "return temperature reading even if device stops after answering" in {
    val requester = TestProbe()
  ...
  "" should "return DeviceTimedOut if device does not answer in time" in {
    val requester = TestProbe()
  ...
  "" should "be able to collect temperatures from all active devices" in {
    val probe = TestProbe()
    val groupActor = system.actorOf(DeviceGroup.props("group"))
